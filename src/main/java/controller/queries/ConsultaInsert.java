    ...
    public void ejecutarInsert(String sql) {
        for (Connection conn : conexionesSQL.values()) {
            if (ejecutarActualizacionSQL(conn, sql)) {
                System.out.println("✅ Insert realizado en SQL Server.");
                return;
            }
        }
        ...
        for (Session session : conexionesNeo4j.values()) {
            if (ejecutarActualizacionNeo4j(session, sql)) {
                System.out.println("✅ Insert realizado en Neo4j.");
                return;
            }
        }

        System.err.println("⚠️ No se pudo insertar en ninguna base de datos.");
    }
